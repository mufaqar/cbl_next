...
function OverView({ uniqueServiceType, type, city, state, totalProviderCount, allProviders }: any) {
    ...
    return (
        ...
        <p className='text-xl font-[Roboto] mt-5'>
            As of the time this page was written, {city === '' ? <span className='uppercase'>{state}</span> : city}  has {totalProviderCount} {formatType(type)} Service Providers offering various types of {formatType(type)} plans to its residents. You’ll likely have options from
            {
