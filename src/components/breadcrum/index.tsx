                    ...
                    </li>
                    {
                         bredcrumList?.map((item:any, idx:number) => (
                              <li className="inline-flex items-center" key={idx}>
                                   <Link className={`flex cursor-pointer items-center text-sm  ${idx+1 === bredcrumList.length ? '!text-gray-300' : 'text-gray-500 hover:text-blue-600 focus:outline-none'}`} href={`/${item}`}>
                                        {item?.name}
                                        {
